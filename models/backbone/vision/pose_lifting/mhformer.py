...
from .poseformer import Attention, Block, Mlp
...
class MHPoseFormer(nn.Module):
    def __init__(self, num_frame=81, num_joints=17, layers=3, channel=512, d_hid=1024):
        ...
        self.Transformer_encoder_1 = _TransformerEncoder(
            4, num_frame, num_frame * 2, length=2 * num_joints, h=9
        )
        self.Transformer_encoder_2 = _TransformerEncoder(
            4, num_frame, num_frame * 2, length=2 * num_joints, h=9
        )
        self.Transformer_encoder_3 = _TransformerEncoder(
            4, num_frame, num_frame * 2, length=2 * num_joints, h=9
        )
        ...
        # SHR & CHI
        self.Transformer_hypothesis = _TransformerHypothesis(
            layers, channel, d_hid, length=num_frame
        )
